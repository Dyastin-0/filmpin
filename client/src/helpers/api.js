...
export const fetchMovies = async (api, genres = '', page = '1', sortBy = 'vote_count') => {
	const response = await api.get(`/movies/discover?genres=${genres}&sort_by=${sortBy}&page=${page}`);
	return response.data.results;
};

export const fetchShows = async (api, genres = '', page = '1', sortBy = 'vote_count') => {
	const response = await api.get(`/tvshows/discover?genres=${genres}&sort_by=${sortBy}&page=${page}`);
	return response.data.results;
};

export const fetchDiscovery = async (api, target, genres, sortBy = 'vote_count', page = '1') => {
	const response = await api.get(`/${target}/discover?genres=${genres}&sort_by=${sortBy}&page=${page}`);
	return response.data;
};
...
export const fetchTvShowEpisodeDetails = async (api, id, seasonNumber, episodeNumber) => {
	const response = await api.get(`/tvshows/season/episode?tvshow_id=${id}&season_number=${seasonNumber}&episode_number=${episodeNumber}`);
	return response.data;
}

export const fetchTvShowSeason = async (api, id, seasonNumber) => {
	const response = await api.get(`/tvshows/season?tvshow_id=${id}&season_number=${seasonNumber}`);
	return response.data;
}

export const fetchTvShowSeasonVideos = async (api, id, seasonNumber) => {
	const response = await api.get(`/tvshows/season/videos?tvshow_id=${id}&tvshow_season=${seasonNumber}`);
	return response.data.results;
}