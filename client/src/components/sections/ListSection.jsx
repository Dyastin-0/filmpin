...
const ListSection = ({ userData }) => {
  ...
  if (!token) {
    <motion.section className="flex justify-center items-center gap-4 w-full bg-accent rounded-md">
      <h1 className="text-xs text-primary-foreground font-semibold">{`Sign in to view ${userData?.username}'s lists.`}</h1>
    </motion.section>;
  }

  return (
    <motion.section className="relative flex flex-col gap-4 w-full z-10">
